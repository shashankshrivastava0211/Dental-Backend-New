...
const getTodayAppointmentsForDoctor = async (doctorId) => {
  ...
  try {
    ...
    console.error(
      `Error fetching today's appointments for doctor ${doctorId}:`,
      error
    );
    return [];
  }
...
const sendDailySummariesToAllDoctors = async () => {
  try {
    ...
    const results = await Promise.allSettled(
      activeDoctors.map((doctor) => sendDailySummaryToDoctor(doctor._id))
    );
    ...
    console.log(
      `Daily summaries sent: ${successful} successful, ${failed} failed`
    );
...
const sendUpcomingBookingsToDoctor = async (doctorId, daysAhead = 7) => {
  try {
    ...
    if (upcomingAppointments.length > 0) {
      await sendUpcomingBookingsNotification(
        doctor.email,
        doctor.name,
    ...
    console.error(
      `Error sending upcoming bookings to doctor ${doctorId}:`,
      error
    );
    return { success: false, error: error.message };
  }
