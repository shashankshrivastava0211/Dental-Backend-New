...
const sendSubscriptionNotification = async (doctorEmail, doctorName, subscriptionDetails) => {
    try {
      ...
      `,
        ...
        console.log(`Subscription notification email sent to Dr. ${doctorName}: ${info.messageId}`);
...
const sendUpcomingBookingsNotification = async (doctorEmail, doctorName, upcomingBookings) => {
    try {
        const transporter = createTransporter();
        const mailOptions = {
            from: process.env.EMAIL_USER,
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;">
          <div style="background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="margin-bottom: 25px;">
              ...
              <p style="color: #666; line-height: 1.6; margin: 0;">
                You have ${upcomingBookings.length} upcoming appointment(s) that require your attention.
              </p>
            </div>
              ...
              ${upcomingBookings
                ...
                <div style="border-left: 4px solid #007bff; padding-left: 15px; margin-bottom: 15px;">
                  ...
                  <p style="color: #666; margin: 0; font-size: 14px;">
                    Phone: ${booking.phoneNo} | Age: ${booking.age} | Gender: ${booking.gender}
                  </p>
                  ${booking.description
      ...
      `,
        ...
        const info = await transporter.sendMail(mailOptions);
        console.log(`Upcoming bookings notification sent to Dr. ${doctorName}: ${info.messageId}`);
        return { success: true, messageId: info.messageId };
    }
    catch (error) {
...
const sendNewAppointmentNotification = async (doctorEmail, doctorName, appointmentDetails) => {
    try {
        const transporter = createTransporter();
        const mailOptions = {
            from: process.env.EMAIL_USER,
          ...
          <div style="background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="background-color: #e8f5e8; padding: 20px; border-radius: 8px; margin-bottom: 25px;">
              ...
              <div style="border-left: 4px solid #28a745; padding-left: 15px;">
                ...
                <p style="color: #666; margin: 0 0 5px 0; font-size: 14px;">
                  Phone: ${appointmentDetails.phoneNo} | Age: ${appointmentDetails.age} | Gender: ${appointmentDetails.gender}
                </p>
      ...
      `,
        ...
        const info = await transporter.sendMail(mailOptions);
        console.log(`New appointment notification sent to Dr. ${doctorName}: ${info.messageId}`);
        return { success: true, messageId: info.messageId };
    }
    catch (error) {
...
const sendDailyAppointmentsSummary = async (doctorEmail, doctorName, appointmentsSummary) => {
    try {
        const transporter = createTransporter();
        const mailOptions = {
            from: process.env.EMAIL_USER,
            ...
            <div style="background-color: #e8f5e8; padding: 20px; border-radius: 8px; margin-bottom: 25px;">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div style="text-align: center; padding: 15px; background-color: #ffffff; border-radius: 6px;">
                  <h4 style="color: #28a745; margin: 0 0 5px 0; font-size: 24px;">${appointmentsSummary.todayCount}</h4>
                  <p style="color: #666; margin: 0; font-size: 14px;">Today's Appointments</p>
                </div>
                <div style="text-align: center; padding: 15px; background-color: #ffffff; border-radius: 6px;">
                  <h4 style="color: #007bff; margin: 0 0 5px 0; font-size: 24px;">${appointmentsSummary.pendingCount}</h4>
